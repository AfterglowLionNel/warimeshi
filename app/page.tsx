...
const HOME_FAQ_ITEMS = [
  {
    ...
    answerNode: (
      <p>
        warimeshi は <strong>飲み会・旅行・食事会・タクシー代</strong> の割り勘計算を無料で行えるウェブアプリです。
        注文ごとに金額を記録して、<strong>均等割り</strong>や<strong>傾斜配分</strong>で公平に精算できます。
        グループ共有用の招待リンクを発行できるので、幹事ひとりで全員の注文を打ち込む必要はありません。
        <strong>インストール不要・ログイン不要</strong>で、スマートフォンのブラウザからすぐに利用できます。
      </p>
    ),
  },
  {
    ...
    answerNode: (
      <p>
        warimeshi は合計金額を人数で割った<strong>余り (端数)</strong>を自動的に検出します。
        たとえば 3,200 円を 3 人で割ると <strong>1,066 円 × 2 名・1,068 円 × 1 名</strong> のように、
        端数 +1 円を負担する人を明示して表示します。誰がいくら払うか一目で分かるので、幹事が頭の中で計算する必要はありません。
      </p>
    ),
  },
  {
    ...
    answerNode: (
      <p>
        <strong>/taxi ページの「区間別」モード</strong> を使うと、出発地点 → 1人目降車地 → 2人目降車地 ... の順に距離と降りる人数を入力できます。
        距離料金は<strong>その区間に乗っている人数で按分</strong>されるため、先に降りる人は短い区間ぶん、最後まで乗る人は全区間ぶんを負担する公平な計算になります。
        <strong>初乗り料金・迎車料金・深夜割増・運転代行の長距離割増</strong>も設定で反映できます。
      </p>
    ),
  },
  {
    ...
    answerNode: (
      <p>
        <strong>「グループで始める」</strong> からテーブルを作成すると、招待用の <strong>URL と QR コード</strong> が発行されます。
        参加者がそのリンクを開くだけで自分のスマートフォンから注文を入力できるようになり、<strong>全員の画面にリアルタイムで反映</strong>されます。
        幹事は紙やメモを取らずに済み、お会計のときには各自の支払額が自動で表示されます。
      </p>
    ),
  },
  {
    ...
    answerNode: (
      <p>
        <strong>登録もログインも不要</strong>で、すべての機能を<strong>無料</strong>で利用できます。
        Google などでログインするとテーブルの履歴が残せますが、ゲストのまま使うこともできます。
        料金やサブスクリプションは一切なく、広告の表示も最小限です。
      </p>
    ),
  },
  {
    ...
    answerNode: (
      <p>
        <strong>ソロモード</strong> はひとりで自分の注文だけを素早く記録するモードで、ログイン不要・即時起動できます。
        <strong>グループモード</strong> は複数人で同じテーブルを共有し、招待リンク経由で各自が自分の注文を入力できる<strong>リアルタイム同期モード</strong>です。
        少人数の飲み会から大人数の宴会・旅行まで対応できます。
      </p>
    ),
  },
  {
    question: "お楽しみ調整・ルーレット機能とは？",
    answer:
      "グループモードでは、お会計をちょっと盛り上げる「お楽しみ調整」を任意で使えます。(1) 端数だけルーレット: 100円未満の端数をランダムで誰か1人に押し付ける、(2) 1人だけ500円引き: ランダムにラッキー割対象を抽選、(3) 全額負担ルーレット: 止まった人が全額をおごる罰ゲーム風モード、(4) 幹事ボーナス: 払い出し担当 (幹事) を500円割引、の4種類。スピン演出と紙吹雪エフェクト付きで、飲み会の罰ゲームや締めの一発勝負としても楽しめます。もちろん使わずに通常の均等割り精算も可能です。",
    answerNode: (
      <>
        <p>
          グループモードでは、お会計をちょっと盛り上げる <strong>「お楽しみ調整」</strong> を任意で使えます。
        </p>
        <ol className="space-y-1.5 pl-0 list-none">
          <li>
            <strong>① 端数だけルーレット</strong>
            <br />
            100円未満の端数をランダムで誰か1人に押し付ける
          </li>
          <li>
          ...
          <li>
            ...
            <br />
            止まった人が全額をおごる罰ゲーム風モード
          </li>
          <li>
            <strong>④ 幹事ボーナス</strong>
            <br />
            払い出し担当 (幹事) を500円割引
          </li>
        </ol>
        <p>
          <strong>スピン演出と紙吹雪エフェクト</strong>付きで、飲み会の<strong>罰ゲーム</strong>や締めの一発勝負としても楽しめます。
          もちろん使わずに通常の均等割り精算も可能です。
        </p>
      </>
    ),
  },
